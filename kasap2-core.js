...
const LEGACY_INDEX_MAP={0:29,2:27,3:38,4:37,5:28,8:31,9:32,10:42,11:39,17:62,18:45,19:43,20:63,24:4,25:1,26:10,27:6,45:60,46:14,48:15,50:16,51:17,52:17,53:19,55:61,58:20};
function migrateOldOrders(o){const n={};if(!o||typeof o!=="object")return n;Object.entries(o).forEach(([k,v])=>{const ni=LEGACY_INDEX_MAP[Number(k)];if(Number.isInteger(ni))n[ni]=v});return n}
function norm(c,legacy=false){return {name:String(c?.name||""),orders:legacy?migrateOldOrders(c?.orders):(c&&typeof c.orders==="object"&&c.orders)||{},page:Number.isInteger(c?.page)&&c.page>=1&&c.page<=10?c.page:null}}
(function load(){try{const raw=localStorage.getItem(STORAGE_KEY);if(raw){const s=JSON.parse(raw);if(s){customers=Array.isArray(s.customers)?s.customers.map(c=>norm(c,false)):[];selected=Number.isInteger(s.selected)?s.selected:0;lang=s.lang==="tr"?"tr":"de";company=s.company||company;baseFile=s.baseFile||baseFile}}else{const old=localStorage.getItem(OLD_STORAGE_KEY);if(old){const s=JSON.parse(old);if(s){customers=Array.isArray(s.customers)?s.customers.map(c=>norm(c,true)):[];selected=Number.isInteger(s.selected)?s.selected:0;lang=s.lang==="tr"?"tr":"de";company=s.company||company;baseFile=s.baseFile||baseFile}}}}catch(e){console.error(e)}if(!customers.length)customers=DEFAULT_CUSTOMERS.map(name=>({name,orders:{},page:null}));if(selected<0||selected>=customers.length)selected=0})();
function t(){return T[lang]}
function save(){company=companyName.value||company;baseFile=fileName.value||baseFile;localStorage.setItem(STORAGE_KEY,JSON.stringify({articleVersion:ARTICLE_LAYOUT_VERSION,customers,selected,lang,company,baseFile}))}
...
function renderItems(){if(selected<0||!customers[selected]){items.textContent=t().first;return}const q=(articleSearch.value||"").trim().toLowerCase();let html=`<table class="items-table"><thead><tr><th class="item">${t().ware}</th><th class="num">kg</th><th class="num">${t().count}</th></tr></thead><tbody>`;let left=false,right=false;ARTICLES.forEach((a,i)=>{const name=articleName(i);if(q&&!name.toLowerCase().includes(q)&&!a[0].toLowerCase().includes(q)&&!a[1].toLowerCase().includes(q))return;const isLeft=i<PAPER_LEFT_COUNT;if(isLeft&&!left){html+=`<tr class="group-row paper-side"><td colspan="3">${escapeHtml(t().left)}</td></tr>`;left=true}if(!isLeft&&!right){html+=`<tr class="group-row paper-side"><td colspan="3">${escapeHtml(t().right)}</td></tr>`;right=true}const o=orderAt(customers[selected].orders?.[i]);html+=`<tr><td>${escapeHtml(name)}</td><td><input type="number" min="0" step="0.01" inputmode="decimal" value="${escapeHtml(o.kg)}" oninput="saveOrder(${i},'kg',this.value)"></td><td><input type="number" min="0" step="1" inputmode="numeric" value="${escapeHtml(o.count)}" oninput="saveOrder(${i},'count',this.value)"></td></tr>`});items.innerHTML=html+'</tbody></table>'}
