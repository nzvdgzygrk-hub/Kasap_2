...
function renderCustomers(){customerSelect.innerHTML="";if(!customers.length){customerSelect.innerHTML=`<option>${t().none}</option>`;return}customers.forEach((c,i)=>{const o=document.createElement("option");o.value=i;o.textContent=c.name+(c.page?` · Sayfa ${c.page}`:"");if(i===selected)o.selected=true;customerSelect.appendChild(o)})}
...
function renderAssignments(){
 ...
 customers.forEach((c,i)=>{html+=`<tr><td>${escapeHtml(c.name)}</td>`;for(let p=1;p<=10;p++)html+=`<td><input type="radio" name="page_${i}" aria-label="${escapeHtml(c.name)} Sayfa ${p}" ${c.page===p?'checked':''} onchange="setPage(${i},${p})"></td>`;html+='</tr>'});
 assignmentGrid.innerHTML=html+'</tbody></table>';const missing=customers.filter(c=>!c.page).length;assignmentStatus.className='status '+(missing?'warn':'ok');assignmentStatus.textContent=missing?`${missing} ${t().unassigned}`:t().assigned;
}
